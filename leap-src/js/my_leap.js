...
function GetSettings() {
    if (DEBUG) console.log('Updating settings');
    chrome.storage.local.get(
        [
            'setupDone', 'notificationPos',
            'extensionOn', 'startOn',
            'errorPos', 'scrollOn',
            'historyOn', 'tabOn',
            'refreshOn', 'zoomOn',
            'continuousScroll', 'scrollThresholdX',
            'scrollThresholdY', 'scrollStepX',
            'scrollStepY', 'connectionTimeOut',
            'fpsScaleFactor', 'discreteActionDelay',
            'refreshThreshold', 'historyThreshold',
            'firstSettings', 'newCenter',
            'zoomScale'
        ],
        function (items) {
            if (typeof (items.setupDone) === 'undefined') {
                ...
                if (typeof (items.firstSettings) === 'undefined') {
                    if (DEBUG) console.log('Setting first run settings');
                    chrome.storage.local.set({
                        extensionOn: true,
                        startOn: true,
    ...
    function getScrollMax(axis) {
        ...
        if (axis === 'x')
            return Math.max(
                body.scrollWidth, body.offsetWidth, html.clientWidth,
...
setInterval(function () {
    if (appSettings.extensionOn && attached && leap_status) GetSettings()
}, 10000);
...
function CheckConnection() {
    if (tab_has_focus && attached) {
        ...
        if (currentTime - lastCheckTime - last_focus_time >
...
function initStatus() {
    if (attached) {
        if (DEBUG) console.log('Start init check');
...
function MessagingHandler() {
    ...
    try {
        chrome.runtime.onMessage.addListener(function (
            ...
            if (DEBUG)
                console.log(
                    'onMessage data: ' + JSON.stringify(request) + ' ' +
            ...
            if (request.popUpAction === 'disconnect') {
                ...
                if (leap_status === 'Port disconnected')
                    popup_command_timeout = setTimeout(function () {
                        FadeStatusImg(false)
                    }, stopNotificationTimeout * 1000);
                sendResponse({
                    leap_status: leap_status,
                    attached: attached
                });
...
controller.loop(function (frame) {
    ...
    if (!appSettings.extensionOn) return;
    ...
    leap_status = 'Port connected';
    if (tab_has_focus) chrome.storage.local.set({
        leap_status: leap_status
    });
    attached = true;
    streaming = true;
    ...
    if (frame.valid && frame.pointables.length > 0) {
        ...
        if (setupConfirm) {
            ...
            chrome.storage.local.set({
                newCenter: JSON.parse(appSettings.newCenter)
            });
            chrome.storage.local.set({
                setupDone: appSettings.setupDone
            });
            setupConfirm = false;
        ...
        for (var i = 0; i < 5; i++) {
            if (fingersList[i].extended) extendedFingers++;
        }
        ...
        if (extendedFingers >= 4) {
            // scroll gesture
            // if there's 5 extended fingers go with scroll or history navigation
            // gesture
                    ...
                    if (gesture.type === 'swipe') {
                        ...
                        if (isHorizontal && curHistoryCount > 0) {
                            if (gesture.direction[0] > 0 &&
                                lastAction !== 'move_history_left') {
                                action = 'move_history_right';
                            } else if (
                                gesture.direction[0] < 0 &&
        ...
        // refresh gesture with index finger
            ...
            appSettings.refreshOn && extendedFingers === 1 &&
            fingersList[1].extended && lastExtendedFingers === 1 &&
        ...
        // tab move gesture with two fingers
            ...
            appSettings.tabOn && extendedFingers === 2 &&
            frame.gestures.length > 0) {
            if (DEBUG) console.log(frame.gestures);
            gesture = frame.gestures[0];
            if (gesture.type === 'swipe') {
                isHorizontal =
                    Math.abs(gesture.direction[0]) > Math.abs(gesture.direction[1]);
                if (isHorizontal && lastAction !== 'move_tab_left' &&
                    lastAction !== 'move_tab_right') {
                    if (gesture.direction[0] > 0) {
        ...
        // zoom factor is 10%
            ...
            appSettings.zoomOn && extendedFingers === 1 &&
            lastExtendedFingers === 1 && fingersList[0].extended &&
            lastAction !== 'zoom' &&
            Math.abs(frameDiff.z) > appSettings.scrollThresholdY) {
            var thumbPos = fingersList[0].dipPosition;
            var IndexPos = fingersList[1].dipPosition;
            var distance = Math.sqrt(
                Math.pow(thumbPos[0] - IndexPos[0], 2) +
        ...
        // log action
        if (DEBUG) console.log('current action: ' + action);
        ...
        switch (action) {
            ...
            case 'zoom':
                zoomPage(zoomFactor);
                if (zoomFactor >= 0) UpdateStatusImage('zoomOut');
                if (zoomFactor < 0) UpdateStatusImage('zoomIn');
                curHistoryCount = 0;
                break;
            case 'refresh':
            ...
            case 'move_history_right':
                ...
                }, appSettings.discreteActionDelay);
                curHistoryCount = 0;
                break;
            case 'move_history_left':
                ...
                // since it checks there if the switching was successful or not from
            ...
            case 'none':
                ...
                if (leap_status === 'Port connected') {
                    if (DEBUG) console.log('connected image from case none');
                    UpdateStatusImage('connected');
                } else {
                    if (DEBUG) console.log('disconnected image from case none');
                    UpdateStatusImage('disconnected');
                }
                // curHistoryCount = 0;
                break;
    ...
    function navigateHistory(direction) {
        if (direction === 'right') {
            if (DEBUG) console.log('Next history page');
            history.forward();
        } else if (direction === 'left') {
            if (DEBUG) console.log('Previous history page');
            history.back();
        }
    }
    ...
    function RefreshPage() {
        if (DEBUG) console.log('Refreshing page');
        location.reload();
    }

    // new center setup function
    function NewCenterSetup() {
        if (DEBUG) console.log('Append interactionBox setup DOM');
        $('body').append(
            '<dialog style="position:fixed" class="modal" id=' +
            INTERACTION_BOX_SETUP_DOM_NAME + '>' +
        ...
        document.getElementById('cancelModal')
            .addEventListener('click', function (e) {
                if (DEBUG) console.log('Center setup canceled');
                setupDialog.close();
            });
        document.getElementById('okModal').addEventListener('click', function (e) {
            if (DEBUG) console.log('Center changed successfully!');
            setupConfirm = true;
...
function UpdateStatusImage(action) {
    ...
    if (action === 'scroll') {
        ...
        DOMPosition();
        if (action === 'zoomIn') imgURL = zoomInImage;
        if (action === 'zoomOut') imgURL = zoomOutImage;
...
function PushNotification(message, color) {
    ...
    function StatusMessage(message, color) {
        ...
        if (DEBUG) console.log('Push notification');
    }
...
function AddDOMElement() {
    ...
    $('body').append(
        '<div id=' + STATUS_DIV_DOM_NAME + '>' +
        '<img id=' + STATUS_IMAGE_DOM_NAME +
        ' src="" alt="scrolling" width="72" height="72"/></div>');
    ...
    $(StatusAppendPos)
        .append('<div id=' + NOTIFICATION_MESSAGE_DOM_NAME + '></div>');

    // log event
    if (DEBUG) console.log('Added status image and notification message DOM');
...
// leap port and service connected and ready, this doesn't mean the device is
...
controller.on('disconnect', function () {
    ...
    // log
    if (DEBUG) console.log('Port disconnect');
});
...
controller.on('deviceStreaming', function (deviceInfo) {
    // this line only to support Linux, since the deviceAttached is not triggered
    // for some reason
    deviceAttached(deviceInfo);

    // set runtime flag
    streaming = true;
    if (DEBUG) console.log('deviceStreaming', deviceInfo);
});
controller.on('deviceStopped', function (deviceInfo) {
    // this line only to support Linux, since the deviceRemoved is not triggered
    // for some reason
    deviceRemoved(deviceInfo);

    // set runtime flag
    streaming = false;
    if (DEBUG) console.log('deviceStopped', deviceInfo);
});
...
function deviceAttached(deviceInfo) {
    ...
    clearInterval(messageInterval);
    UpdateStatusImage('connected');
    if (DEBUG) console.log('deviceAttached', deviceInfo);
}

// method to handle event when the device is physically removed
function deviceRemoved(deviceInfo) {
    if (DEBUG) console.log('user_disconnect: ' + user_disconnect);
    if (!user_disconnect) {
        attached = false;
        ...
        if (DEBUG) console.log('deviceRemoved', deviceInfo);
    }
    UpdateStatusImage('disconnected');
    if (DEBUG) console.log('Connection lost!');
}

controller.on('streamingStarted', function (deviceInfo) {
    if (DEBUG) console.log('streamingStarted', deviceInfo);
});

controller.on('streamingStopped', function (deviceInfo) {
    if (DEBUG) console.log('streamingStopped', deviceInfo);
});